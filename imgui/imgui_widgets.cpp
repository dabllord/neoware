...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y), window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2));
    ...
    if (*v) {
        RenderFrame2(check_bb.Min - ImVec2(1, 2), check_bb.Max - ImVec2(1, 2), true, 2.5f, true);
        RenderCheckMark(window->DrawList, ImVec2((total_bb.Min.x + check_bb.Max.x) / 2 - 5.5f, (total_bb.Min.y + total_bb.Max.y) / 2 - 6.5), ImColor(29, 29, 29), 9.0f);
    }
    else {
        RenderFrame(check_bb.Min - ImVec2(1, 2), check_bb.Max - ImVec2(1, 2), ImColor(29, 29, 29), true, 2.5f, true);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    Spacing();
    Spacing();
    Spacing();
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(13, 0), frame_bb.Max + ImVec2(0, 4), ImColor(29, 29, 29, int(255 * fill)), style.FrameRounding);

    //window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(15, 2), ImVec2(max(frame_bb.Min.x + 14, grab_bb.Max.x * 1 + 2), frame_bb.Max.y) + ImVec2(0, 2), ImColor(187, 150, 159, int(255 * fill)), style.FrameRounding);

    window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(15, 2), ImVec2(std::max(frame_bb.Min.x + 14, grab_bb.Max.x * 1 + 2), frame_bb.Max.y) + ImVec2(0, 2), ImColor(187, 150, 159), ImColor(187, 150, 159), ImColor(131, 107, 113), ImColor(131, 107, 113));

    window->DrawList->AddRect(frame_bb.Min + ImVec2(14, 1), frame_bb.Max + ImVec2(-1, 3), ImColor(29, 29, 29, int(255 * fill)), style.FrameRounding);

    window->DrawList->AddRect(frame_bb.Min + ImVec2(13, 0), frame_bb.Max + ImVec2(0, 4), ImColor(41, 41, 41, int(255 * fill)), style.FrameRounding);

    window->DrawList->AddRectFilled(ImVec2(std::max(frame_bb.Min.x + 20 + 3, grab_bb.Max.x * 1 - 5 + 3), frame_bb.Min.y),ImVec2(std::max(frame_bb.Min.x + 14 + 3, grab_bb.Max.x * 1 - 1 + 3), frame_bb.Max.y + 2) + ImVec2(0, 2),
        ImColor(160, 160, 160, int(255 * fill * 1.5f)), style.FrameRounding);


    window->DrawList->AddRect(ImVec2(std::max(frame_bb.Min.x + 20 + 3, grab_bb.Max.x * 1 - 5 + 3), frame_bb.Min.y) - ImVec2(1, 1), ImVec2(std::max(frame_bb.Min.x + 14 + 3, grab_bb.Max.x * 1 - 1 + 3), frame_bb.Max.y + 2) + ImVec2(0, 2) + ImVec2(1, 1),
        ImColor(38, 38, 38, int(255 * fill * 1.5f)), style.FrameRounding);

    //window->DrawList->AddRect(ImVec2(max(frame_bb.Min.x + 20, grab_bb.Max.x * 1 - 5), frame_bb.Min.y - 2),
        //ImVec2(max(frame_bb.Min.x + 14, grab_bb.Max.x * 1 + 2), frame_bb.Max.y + 2) + ImVec2(0, 4), ImColor(38, 38, 38, int(255 * fill * 1.5f)), style.FrameRounding);
...

    {
        ...
        strcpy(temp_buf, text);  // �������� "text" �� ��������� �����
        strcat(temp_buf, value_buf);  // ��������� ���������� value_buf �� ��������� �����

        // �������� ������������ ������ ������� � value_buf
        strcpy(value_buf, temp_buf);

        PushStyleColor(ImGuiCol_Text, ImColor(126, 126, 126, int(255 * fill)).operator ImVec4());
        auto label_buffer_size = CalcTextSize(value_buf, value_buf_end, true);
        RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x - 14, frame_bb.Min.y - g.Font->FontSize - 2), value_buf);
        PopStyleColor();

        PushStyleColor(ImGuiCol_Text, ImColor(202, 202, 202, int(255 * fill)).operator ImVec4());

        if (label_size.x > 0.0f) {
            RenderText(ImVec2(frame_bb.Min.x + 13, frame_bb.Min.y - g.Font->FontSize - 2), label);
        }
        PopStyleColor();
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        RenderNavHighlight(frame_bb, id);
        //RenderFrame(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + 5 + ImVec2(1, 1), GetColorU32(ImGuiCol_FrameBgActive, it_hover->second), true, 3);
        RenderFrame(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(5, 5) + ImVec2(1, 1), ImColor(29, 29, 29), true, 3);
        RenderFrame(frame_bb.Min, frame_bb.Max + ImVec2(5, 5), ImColor(34, 34, 34), true, 3);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(0, 4), rect.Max, bg_color);
                }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min + ImVec2(0,2), cursor_screen_rect.GetBL() + ImVec2(0, 3), GetColorU32(ImGuiCol_Text));
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() + ImVec2(0, border ? 20 : 5), ImGui::GetWindowPos() + size_arg, ImColor(34, 34, 34),10); //bg
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() + ImVec2(0, border ? 20 : 5), ImGui::GetWindowPos() + ImVec2(size_arg.x, 36), ImColor(41, 41, 41), 10, 3); //top
    //parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() + ImVec2(0, border ? 37 : 23), ImGui::GetWindowPos() + ImVec2(size_arg.x, border ? 40 : 25), ImColor(89, 113, 162));
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() + ImVec2(1, border ? 20 : 5 +1), ImGui::GetWindowPos() + size_arg - ImVec2(1,1), ImColor(41, 41, 41), 10);
    parent_window->DrawList->AddLine(ImGui::GetWindowPos() + ImVec2(0, 36), ImGui::GetWindowPos() + ImVec2(size_arg.x - 0.5, 36), ImColor(193, 154, 164)); // seperator
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() + ImVec2(0, border ? 20 : 5), ImGui::GetWindowPos() + size_arg, ImColor(23,23,23), 10);//
    parent_window->DrawList->AddText(ImGui::GetWindowPos() + ImVec2(10, border ? 22 : 7 + 5), ImColor(200, 200, 200), name);
    return ret;
}
